...
def assert_lq(a, b, c):
    print(lt(a, b) and lt(b, c), lt(a, c))
    return lt(a, c) if lt(a, b) and lt(b, c) else True

@pytest.mark.task0_2
@given(small_floats, small_floats, small_floats)
def test_transitive(a: float, b: float, c: float) -> None:
    """Test the transitive property of less-than (a < b and b < c implies a < c)"""
    assert (assert_lq(a, b, c) and assert_lq(a, c, b) and
           assert_lq(b, a, c) and assert_lq(b, c, a) and
           assert_lq(c, a, b) and assert_lq(c, b, a))
...
#    assert mul(1e6, 2e3) == mul(2e3, 1e6)
#    assert mul(-1, 1) == mul(1, -1)
...
def test_other() -> None:
    ...
    assert True#assert_close(0, 0.001)
    #assert not assert_close(0, 0.02)
    #TODO
